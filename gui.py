...
class MainWindow(tk.Tk):
    ...
    def launch(self):
        ...
        for elem in cf.menus_main:
            ...
            for el in elem['unfold']:
                menu.add_command(label=el['label'], command=eval(el['function'], {'self': self}))
    ...
    def request_controler_tournament(self, request):
        """ send a request to the controller to authorize actions """
        authorization = ct.control_request(request)
        if authorization:
            for elem in cf.authorization:
                for key, value in elem.items():
                    if (key == 'name') & (value == request):
            ...
            for elem in cf.authorization:
                for key, value in elem.items():
                    if (key == 'name') & (value == request):
                        mb.showinfo(title=cf.title_error, message=elem['error_message'])

    def get_info_to_controller(self):
        """ send the info entered by user to the controller """
