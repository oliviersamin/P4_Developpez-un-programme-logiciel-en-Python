...
class Controls:
    ...
    def send_order_to_gui(cls):
        """ send order to GUI so that it can update the states of the menus regarding the order """
        ...
        if data == {}:
            ...
            if len(data['players']) == 0:
                ...
                return 'repeat_step'
            elif (len(data['players']) == cf.number_of_players) & (data['rounds'] == []):
                return 'next_step'
    ...
    def update_json_file(cls):
        """ create or update the JSON file which is the link between controller and GUI """
        global tournament
    ...
    def set_match_scores(cls, match_dictionary):
        """ use match_dictionary to get the winner of the match and give scores to players
         match_dictionary = {'match_instance': <Match instance>, 'label': '',
         'choice': ['match nul', <player1>, <player2>], 'result': str(<user choice>)}"""
